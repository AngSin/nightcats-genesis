...
describe("NightCats minting", function () {
	describe("whitelist minting", () => {
		it('should allow minting only till max limit', async () => {
			...
			expect(await nightCats.totalSupply()).to.equal((await nightCats.premintSupply()).add(3n));
			await expect(nightCats.connect(otherAccount)
			...
			expect(await nightCats.totalSupply()).to.equal((await nightCats.premintSupply()).add(5n));
	...
	describe("public mint", () => {
		it('should let public mint', async () => {
			...
			await nightCatsContract.setMaxSupply((await nightCatsContract.totalSupply()).add(4n));
			...
			expect(await nightCatsContract.totalSupply()).to.equal((await nightCatsContract.premintSupply()).add(3n));
			...
			expect(await nightCatsContract.totalSupply()).to.equal((await nightCatsContract.premintSupply()).add(4n));
		});
	});
});
